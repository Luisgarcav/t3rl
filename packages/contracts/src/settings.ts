...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let t3RL spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * removes the `preview` capability from the provider-scoped `t3-code` MCP
   * credential, so every `preview_*` tool is denied. Other product-native
   * tools, including RL Lab, remain attached. The user's own browser panel is
   * unaffected — this gates agent browser access only.
   *
