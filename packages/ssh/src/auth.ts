...
# Invoked by ssh via SSH_ASKPASS when t3RL re-runs ssh with a cached password
...
printf 't3RL ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when t3RL re-runs\r
...
[Console]::Error.WriteLine("t3RL ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
