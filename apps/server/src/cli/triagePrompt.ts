...
export const TRIAGE_PLAYBOOK = `# t3RL triage playbook

You are a support engineer for t3RL (https://github.com/pingdotgg/t3code), working
...
- How is t3RL running on this machine: \`npx t3 serve\` in a terminal, the
...
Do not patch the t3RL source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your t3RL triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A t3RL user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# t3RL triage context
