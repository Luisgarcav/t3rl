...
export interface ProviderServiceLiveOptions {
   ...
   * capability selection unobservable from a unit test; this seam lets a test
   * inspect the exact capability set that was requested.
   */
  readonly issueMcpCredential?: typeof McpSessionRegistry.issueActiveMcpCredential;
  /** Same seam as `issueMcpCredential`, for observing credential replacement. */
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
   ...
   *
   * RL Lab tools are always available to a project thread. Browser tools are
   * added only when the server-authoritative browser preference permits them.
   * Every provider adapter consumes this same session, so capability behavior
   * cannot drift by provider or client surface.
   */
  /**
   ...
   * optional browser capability is attached. Denying is the safe direction —
   * an explicit "off" silently becoming "on" would violate the user's stated
   * choice. RL access does not depend on this setting.
   */
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in t3RL settings.`,
          );
        }
