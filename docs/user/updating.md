# Keeping t3RL in Sync

The t3RL web or desktop app and the server it connects to work best when they use the same
version. If they do not match, t3RL shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                               |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Update server**          | Available for the t3RL Linux background service. Select the button and leave t3RL open while it prepares, tests, restarts, and reconnects.                               |
| **Update the desktop app** | Open the t3RL desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current t3RL server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. t3RL does not update connected
...
If the server instead runs as the t3RL background service, update the service on the host and
...
See [Running t3RL in the Background](./background-service.md) for install, status, and removal
commands.
