...
This guide is for people who want to use more than one Claude setup in t3RL. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install t3RL](./install.md).
...
In t3RL Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means t3RL uses Claude Code's normal config directory.

When you set this field, t3RL points Claude Code at that directory with the
...
t3RL looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where t3RL looks.

Then add another Claude provider in t3RL:
...
t3RL only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so t3RL keeps separate config directories isolated
...
Add or edit a Claude provider in t3RL Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. t3RL stores the value as a server secret and does not
...
t3RL does not need a special Claude Code Router provider. Treat the router as a Claude
