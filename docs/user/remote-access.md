...
Use this when you want to connect to a t3RL server from another device such as a phone, tablet, or separate desktop app.
...
run a headless server from the CLI, or have the desktop app launch t3RL over SSH.
...
Use this when you want the desktop app to start or reuse t3RL on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote t3RL server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual t3RL server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.t3/ssh-launch/<host-key>/`, starts or reuses a remote t3RL server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. t3RL uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, t3RL first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path t3RL uses:
...
When the t3RL web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: t3RL may
...
see [Keeping t3RL in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running t3RL in the Background](./background-service.md).
...
Hosted pairing does not proxy traffic through t3RL. The browser still connects directly to the backend URL in the pairing link.
...
### Deregister a t3RL Connect Environment

Open your account menu and choose **t3RL Connect** to see every environment registered to your
account. On mobile, open **Settings** → **t3RL Connect**. Choose **Deregister** to revoke an
environment's t3RL Connect access, remove any managed tunnel, and free its host space.
