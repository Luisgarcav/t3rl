...
This guide is for people who want to use more than one Codex account in t3RL. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install t3RL](./install.md).
...
- both accounts can see the same t3RL/Codex sessions
...
In t3RL Settings, name it something obvious:
...
In t3RL Settings, add another Codex provider:
...
t3RL shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, t3RL treats it
